...
class Auth extends \fast\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        //     return false;
        // }
        if ($admin->dietime && $admin->dietime<time()) {
            $this->setError('该账号已到期');
            return false;
        }
        ...
        // 保存到 Token 表（用于多设备限制）
        Token::set($admin->token, $admin->id, $keeptime);


        $count = \think\Db::name('user_token')->where('user_id', $admin->id)->count();
        if ($admin->num && $count > intval($admin->num)) {
   
            Token::rm($admin->token);
            $this->setError('账号已经超出同时在线设备，请联系管理员增加');
            return false;
        }
