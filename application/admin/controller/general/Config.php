...
class Config extends Backend
{
    ...
    {
        // 确保客服系统基址配置项存在（用于 Kefu.php 拼装客服系统 URL）
        $this->ensureKefuBaseUrlConfig();
    ...
    /**
     * 确保客服系统基址配置项存在，避免后台无法配置/读取
     */
    protected function ensureKefuBaseUrlConfig()
    {
        // 这里用模型查询，自动带表前缀（fa_config）
        $exists = $this->model->where('name', 'kefu_base_url')->find();
        if ($exists) {
            return;
        }

        // 放到已存在的基础配置分组，避免新增分组导致 configgroup 不一致
        $data = [
            'name'    => 'kefu_base_url',
            'group'   => 'basic',
            'title'   => '客服系统地址',
            'tip'     => '例如：https://kefu.example.com（建议HTTPS，跨域嵌入通常需要SameSite=None;Secure）',
            'type'    => 'string',
            'visible' => '',
            'value'   => '',
            'content' => '',
        ...
        // 只插入，不强制刷新 site.php（保存配置时 edit/add 本身会 refreshFile）
        try {
            $this->model->create($data);
        } catch (\Exception $e) {
            // 不阻断页面渲染，但给出明确错误，方便定位权限/只读文件系统等问题
            throw new Exception('自动创建配置项 kefu_base_url 失败：' . $e->getMessage());
        }
    }
